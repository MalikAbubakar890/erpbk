{{-- Advanced Column Control Panel Component --}}
@php
    ...
    <div class="column-control-body">
        ...
        <div class="mb-4">
            ...
            <div class="column-list" id="columnList">
                ...
                @if(!$isActionColumn)
                <div class="column-item" data-column-index="{{ $index }}" data-column-key="{{ $columnKey }}">
                    <div class="d-flex align-items-center p-2 border rounded mb-2 bg-light">
                        ...
                        <label class="form-check-label flex-grow-1" for="col_{{ $index }}">
                            {{ $column['title'] ?? $column['name'] ?? 'Column ' . ($index + 1) }}
                        </label>
        ...
        <div class="column-stats mt-4 p-3 bg-light rounded">
            <small class="text-muted">
                <strong>Visible:</strong> <span id="visibleCount">{{ count($tableColumns) }}</span> of <span id="totalCount">{{ count($tableColumns) }}</span> columns
            </small>
        </div>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const ColumnController = {
            ...
            resetColumns() {
                ...
                fetch('{{ route("user-table-settings.reset") }}', {
                        method: 'DELETE',
                        headers: {
            ...
            saveToDatabase(settings) {
                fetch('{{ route("user-table-settings.save") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
            ...
            loadUserSettings() {
                ...
                fetch(`{{ route("user-table-settings.get") }}?table_identifier=${encodeURIComponent(this.tableIdentifier)}`, {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
