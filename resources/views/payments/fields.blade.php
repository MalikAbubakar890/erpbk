...
<div class="form-group col-sm-6">
  ...
  <select name="bank_id" id="bank_id" class="form-control select2">
    ...
    <option value="{{ $bank->id }}" {{ old('bank_id', isset($payment) ? $payment->bank_id : '') == $bank->id ? 'selected' : '' }}>{{ $bank->name }}</option>
    @endforeach
  </select>
</div>
<!-- Account Type Field -->
<div class="form-group col-sm-6">
  ...
  <select name="account_type" onchange="selectHeadAccount(this.value)" id="account_type" class="form-control select2">
    ...
    <option value="{{ $type }}" {{ old('account_type', isset($payment) ? $payment->account_type : '') == $type ? 'selected' : '' }}>{{ $label }}</option>
    @endforeach
  </select>
</div>
...
<div class="form-group col-sm-6 mt-3">
  ...
  <div class="form-check">
    ...
    <input type="checkbox" name="status" id="status" class="form-check-input" value="1" @isset($payment) @if($payment->status == 1) checked @endif @else checked @endisset/>
...
<script type="text/javascript">
  function selectHeadAccount(id) {
    if (id) {
      $.ajax({
        type: 'get',
        url: '{{ url("payments/headbytype") }}/' + id,
