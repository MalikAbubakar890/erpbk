@extends('layouts.app')
@section('title','Rider Monthly Report')
@section('content')
<style>
    ...
    <section class="content">
        <div class="row">
            <div class="col-12">
                ...
                ['data' => 'opening_balance', 'title' => 'Opening Balance'],
                ['data' => 'monthly_debit', 'title' => 'Monthly Debit'],
                ['data' => 'monthly_credit', 'title' => 'Monthly Credit'],
                ['data' => 'net_activity', 'title' => 'Net Activity'],
                ['data' => 'closing_balance', 'title' => 'Closing Balance'],
                ...
                'tableIdentifier' => 'rider_monthly_report_table'
                ])

                <div class="card rounded-0">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div class="card-title">
                            <h4 class="m-0">Monthly Report Table</h4>
                        </div>
                        ...
                        <div id="totalsBar" class="mb-2" style="display:none;">
                            <div class="totals-cards">
                                <div class="total-card total-opening">
                                    ...
                                    <div class="value" id="monthly_total_opening">0.00</div>
                                </div>
                                <div class="total-card total-amount">
                                    <div class="label"><i class="fa fa-arrow-down"></i> Monthly Debit</div>
                                    <div class="value" id="monthly_total_debit">0.00</div>
                                </div>
                                <div class="total-card total-balance">
                                    <div class="label"><i class="fa fa-arrow-up"></i> Monthly Credit</div>
                                    <div class="value" id="monthly_total_credit">0.00</div>
                                </div>
                                <div class="total-card total-debit">
                                    <div class="label"><i class="fa fa-wave-square"></i> Net Activity</div>
                                    <div class="value" id="monthly_total_net">0.00</div>
                                </div>
                                <div class="total-card total-credit">
                                    <div class="label"><i class="fa fa-scale-balanced"></i> Closing Balance</div>
                                    <div class="value" id="monthly_total_closing">0.00</div>
                                </div>
                            </div>
                        </div>
                        <table id="dataTableBuilder" class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th title="Opening Balance" style="text-align: right;">Opening Balance</th>
                                    <th title="Monthly Debit" style="text-align: right;">Monthly Debit</th>
                                    <th title="Monthly Credit" style="text-align: right;">Monthly Credit</th>
                                    <th title="Net Activity" style="text-align: right;">Net Activity</th>
                                    <th title="Closing Balance" style="text-align: right;">Closing Balance</th>
                                    <th>
        ...
        <form id="filterForm">
            <div class="row">
                ...
                <div class="form-group col-md-12">
                    ...
                    <input type="month" id="billing_month" name="billing_month" value="{{ request('billing_month') ?? date('Y-m') }}" class="form-control" required />
                </div>


                <div class="col-md-12 form-group text-center">
                    <button type="button" class="btn btn-primary pull-right mt-3" onclick="get_data()"><i class="fa fa-filter mx-2"></i> Filter Monthly Data</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(".exportToExcel").click(function() {
            $("#dataTableBuilder").table2excel({
                filename: "RiderMonthly_" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
                fileext: ".xls",
                exclude: ".noExl",
                exclude_img: true,
                exclude_links: true,
    ...
    function applyMonthlyTotals(payload) {
        if (!payload || typeof payload.opening_balance_total === 'undefined') {
            $('#totalsBar').hide();
            return;
        }

        $('#monthly_total_opening').text(formatCurrencyValue(payload.opening_balance_total));
        $('#monthly_total_debit').text(formatCurrencyValue(payload.monthly_debit_total));
        $('#monthly_total_credit').text(formatCurrencyValue(payload.monthly_credit_total));
        $('#monthly_total_net').text(formatCurrencyValue(payload.net_activity_total));
        $('#monthly_total_closing').text(formatCurrencyValue(payload.closing_balance_total));
        $('#totalsBar').show();
    }
