        ...
        <div class="fleet-supervisor-section">
            ...
            <div class="fleet-supervisor-accordion collapsed" id="fleetSupervisorAccordion">
                <div class="fleet-supervisor-cards">
                    ...
                    <?php $__currentLoopData = $fleetSupervisors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fleet): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="fleet-supervisor-card <?php if($fleet == request('fleet_supervisor')): ?> active <?php endif; ?>"
                        data-fleet_supervisor="<?php echo e($fleet); ?>">
                    ...
                    <div class="filter-cards">
                        ...
                        <div class="filter-card">
                            ...
                            <div class="filter-body">
                                <form method="GET" action="<?php echo e(route('riders.index')); ?>" id="statusFilterForm">
                                    <div class="select">
                                        <select class="form-control select2" id="rider_status" name="rider_status[]" multiple onchange="this.form.submit()">
                                            <option value="absconder" <?php echo e(in_array('absconder', request('rider_status', [])) ? 'selected' : ''); ?>>Absconder</option>
                                            <option value="followup" <?php echo e(in_array('followup', request('rider_status', [])) ? 'selected' : ''); ?>>Follow Up</option>
                                        </select>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div class="filter-card">
                            ...
                            <div class="filter-body">
                                <button type="button" class="balance-filter-btn <?php echo e(request('balance_filter') == 'greater_than_zero' ? 'checked' : ''); ?>"
                                    data-balance_filter="greater_than_zero"
                                    id="balanceFilterBtn">
                                    <div class="balance-stats">
                                        <div class="stat-item">
                                            <span class="stat-label">Active</span>
                                            <span class="stat-value"><?php echo e(\App\Models\Riders::where('status', 1)->whereHas('account', function($q) { $q->whereRaw('(SELECT COALESCE(SUM(debit), 0) - COALESCE(SUM(credit), 0) FROM transactions WHERE account_id = accounts.id) > 0'); })->count()); ?></span>
                                        </div>
                                        <div class="stat-divider"></div>
                                        <div class="stat-item">
                                            <span class="stat-label">Inactive</span>
                                            <span class="stat-value"><?php echo e(\App\Models\Riders::where('status', 3)->whereHas('account', function($q) { $q->whereRaw('(SELECT COALESCE(SUM(debit), 0) - COALESCE(SUM(credit), 0) FROM transactions WHERE account_id = accounts.id) > 0'); })->count()); ?></span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="dropdown-content">
                                        <div class="filter-summary">
                                            ...
                                            <div class="summary-item">
                                                ...
                                                <span class="summary-value" id="totalBalanceResults"><?php echo e(\App\Models\Riders::whereHas('account', function($q) { $q->whereRaw('(SELECT COALESCE(SUM(debit), 0) - COALESCE(SUM(credit), 0) FROM transactions WHERE account_id = accounts.id) > 0'); })->count()); ?></span>
                                            </div>
                                        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#filterForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "<?php echo e(route('riders.index')); ?>",
                type: "GET",
                data: formData,
                success: function(data) {
                    $('#table-data').html(data.tableData);
                    let newUrl = "<?php echo e(route('riders.index')); ?>" + (formData ? '?' + formData : '');
        ...
        $('.fleet-supervisor-card').on('click', function() {
            ...
            if (isCurrentlyActive) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#table-data').html(data.tableData);
                        let newUrl = "<?php echo e(route('riders.index')); ?>";
                        history.pushState(null, '', newUrl);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#table-data').html(data.tableData);
                        let newUrl = "<?php echo e(route('riders.index')); ?>" + '?fleet_supervisor=' + encodeURIComponent(fleet_supervisor);
                        history.pushState(null, '', newUrl);
        ...
        $('.balance-filter-btn').on('click', function() {
            ...
            if (isCurrentlyActive || isCurrentlyChecked) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#table-data').html(data.tableData);
                        let newUrl = "<?php echo e(route('riders.index')); ?>" + '?balance_filter=' + encodeURIComponent(balance_filter);
                        history.pushState(null, '', newUrl);
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xammp1\htdocs\erpbk1\resources\views/riders/index.blade.php ENDPATH**/ ?>